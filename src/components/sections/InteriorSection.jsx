  ...
  return (
    <>
      ...
      <div className="app-card ri-card">
        ...
        <button type="button" className="int-btn-add-room" onClick={addInteriorRoom}>
          <Plus size={16} />
          Add Room
        </button>
      </div>
      ...
      {rooms.length === 0 && (
        <div className="app-card int-empty">
          <p className="int-empty__text">No rooms added yet. Tap <strong>Add Room</strong> above to begin.</p>
        </div>
      )}

      {rooms.map(room => (
        <RoomCard key={room.id} room={room} trigPhoto={trigPhoto} />
      ))}
    </>
  )
}
